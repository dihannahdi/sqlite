...
async def test_high_performance_ingestion():
    ...
    total_docs = sum(results.values()) if isinstance(results, dict) else len(results)
    print(f"⚡ Rate: {total_docs / ingestion_time:.1f} docs/sec")
    ...
    print(f"\n🎯 PERFORMANCE SUMMARY:")
    total_docs = sum(results.values()) if isinstance(results, dict) else len(results)
    print(f"   Ingestion Rate: {total_docs / ingestion_time:.1f} docs/sec")
...
def test_batch_loading():
    """Legacy test - now runs the optimized version"""
